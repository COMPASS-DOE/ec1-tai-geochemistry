...
completeset = soilparty %>%
    select(campaign, kit_id, transect_location, loi_perc, bulk_density_g_cm3, gwc_perc, tn_perc, tc_perc, specific_conductance_us_cm, ph, region, percent_clay, percent_sand) %>%
...
points (soilsnmds, col = transpal, pch = 19)
ordiellipse(soilsnmds, groups=completeset$transect_location, col=c("#056009","#8E7941","#021677"))
plot(env1, p.max = 0.05)

env1 <- envfit(soilsnmds, completeset, permu = 999)

NMR_NMS_4 %>% ordiplot(type='none') %>% points("sites") %>% ordiellipse(groups=group2$Location, col=c("#999933","#d7191c","#abdda4") )
