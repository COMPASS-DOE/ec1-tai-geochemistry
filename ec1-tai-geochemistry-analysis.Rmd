...
completeset = soilparty %>% filter(collected == TRUE) %>%
    select(campaign, kit_id, transect_location, loi_perc, bulk_density_g_cm3, gwc_perc, tn_perc, tc_perc, specific_conductance_us_cm, ph, region, percent_clay, percent_sand, plant_litter, visible_minerals, sample_weight_g) %>% select(!ends_with("_flag")) %>% na.omit()
